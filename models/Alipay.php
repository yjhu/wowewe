...
class Alipay
{
	...
	{
		$alipay_config['partner']		= '2088512154429274';
		$alipay_config['key']			= '5q33nc4vhjcx46qbywbaoajm47w1nt5x';
	...
	public static function logResult($word='') {
		U::W($word, Yii::$app->getRuntimePath().'/alipay.log');
		return;
		
		if (is_array($word))
			$word = print_r($word, true);
		else if(is_object($word))		 
			$word = print_r($word, true);						
		else 
			$word = "{$word}";
		$log_file = Yii::$app->getRuntimePath().'/alipay.log';	
		$fp = fopen($log_file,"a");
		flock($fp, LOCK_EX) ;
	...
	public static function getHttpResponsePOST($url, $cacert_url, $para, $input_charset = '') {
		//U::W(['getHttpResponsePOST', $url, $cacert_url, $para, $input_charset]);
